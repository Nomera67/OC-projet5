...
element.wrap(`<div class='item-column mb-4${columnClasses}'></div>`)}else{console.error(`Columns should be defined as numbers or objects. ${typeof columns} is not supported.`)}},moveItemInRowWrapper(element){element.appendTo(".gallery-items-row")},responsiveImageItem(element){if(element.prop("tagName")==="IMG"){element.addClass("img-fluid")}},openLightBox(element,lightboxId){$(`#${lightboxId}`).find(".lightboxImage").attr("src",element.attr("src"));$(`#${lightboxId}`).modal("toggle")},prevImage(lightboxId){let currentImageSrc=$(`#${lightboxId} .lightboxImage`).attr('src');let activeTag=$(".tags-bar span.active-tag").data("images-toggle");let galleryItems=$(".gallery-item").filter(function(){return activeTag==="all"||$(this).data("gallery-tag")===activeTag}).toArray();let currentIndex=galleryItems.findIndex(item=>$(item).attr('src')===currentImageSrc);let prevIndex=(currentIndex-1+galleryItems.length)%galleryItems.length;let prevImageSrc=$(galleryItems[prevIndex]).attr('src');$(`#${lightboxId} .lightboxImage`).attr('src',prevImageSrc)},nextImage(lightboxId){let currentImageSrc=$(`#${lightboxId} .lightboxImage`).attr('src');let activeTag=$(".tags-bar span.active-tag").data("images-toggle");let galleryItems=$(".gallery-item").filter(function(){return activeTag==="all"||$(this).data("gallery-tag")===activeTag}).toArray();let currentIndex=galleryItems.findIndex(item=>$(item).attr('src')===currentImageSrc);let nextIndex=(currentIndex+1)%galleryItems.length;let nextImageSrc=$(galleryItems[nextIndex]).attr('src');$(`#${lightboxId} .lightboxImage`).attr('src',nextImageSrc)},createLightBox(gallery,lightboxId,navigation){gallery.append(`<div class="modal fade" id="${
...
$(".nav-link").removeClass("active active-tag");$(this).addClass("active active-tag");var tag=$(this).data("images-toggle");$(".gallery-item").each(function(){$(this).parents(".item-column").hide();if(tag==="all"){$(this).parents(".item-column").show(300)}else if($(this).data("gallery-tag")===tag){$(this).parents(".item-column").show(300)}})}}})(jQuery)